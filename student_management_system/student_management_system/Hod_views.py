...
from members.models import Course, Session_Year, CustomUser, Student, Staff, Subject,Staff_feedback, Staff_Notification, Staff_leave, Student_Notification, Student_feedback, Student_leave,StaffCopywrite,Staffpatient,Staffdesign,Studentdesign,StudentCopywrite,Studentpatient
...
def SAVE_STUDENT_NOTIFICATION(request):
    if request.method == "POST":
        ...
        std_notification = Student_Notification(
            student_id = student,
            message = message,
        )
        std_notification.save()
        messages.success(request, 'Student Notification Are Successfully Send !')
        return redirect('student_send_notification')
    

def STUDENT_VIEW_DESIGN(request):
    
    return render(request, 'Hod/student_view_design.html')

def STAFF_VIEW_DESIGN(request):
    staff = Staff.objects.all()
    context = {
        'staff': staff,
    }
    return render(request, 'Hod/staff_view_design.html',context)

def STAFF_OPEN_PATIENT(request,id):
    staff = Staff.objects.filter(id = id)
    
    staff_design = Staffdesign.objects.all()
    context = {
        'staff':staff,
        'staff_design': staff_design,
    }
    return render(request, 'Hod/staff_open_patient.html',context)
